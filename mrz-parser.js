...
function formatDate(yymmdd, type) {
  ...
  // MRZ stores dates as YYMMDD. Birth years need century resolution.
  // For birth dates, choose the most recent non-future date, capped to a
  // plausible 120-year age window. Expiry dates keep the original rule.
  let fullYear;
  if (type === "birth") {
    const now = new Date();
    ...
    const candidateDate = new Date(candidate, Number(mm) - 1, Number(dd));
    if (candidateDate > now) candidate -= 100;
    if (currentYear - candidate > 120) candidate += 100;
    fullYear = candidate;
  } else {
    fullYear = (yy >= 60 ? 1900 : 2000) + yy;
  }
